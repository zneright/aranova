...
const UserLayout: React.FC<UserLayoutProps> = ({ children, activeTab = "wallet", userData }) => {
  ...
  return (
    <ThemeContext.Provider value={{ dark, toggleDark: () => setDark((d) => !d) }}>
      {/* CRITICAL MOBILE FIX: 
        Instead of constraining height to 100vh and making inner divs scroll,
        we allow the main window to scroll natively. This is the gold standard for mobile browsers.
      */}
      <div style={{ minHeight: "100vh", background: t.bgPage, color: t.textPrim, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", display: "flex", flexDirection: "column" }}>
        ...
        <header
          style={{
            ...
            padding: "0 24px", position: "sticky", top: 0, zIndex: 100, gap: 12, flexShrink: 0
          }}
        >
          {/* Left: burger + logo */}
        ...
        <div style={{ display: "flex", flex: 1 }}>
          ...
          {/* ── MAIN CONTENT ─────────────────────────────────────────────── */}
          {/* We add extra padding at the bottom ONLY for mobile to ensure content isn't hidden behind the fixed footer */}
          <main style={{ flex: 1, padding: isMobile ? "20px 16px 100px" : "32px", maxWidth: "100%", overflowX: "hidden" }}>
            {children}
          </main>
        </div>
