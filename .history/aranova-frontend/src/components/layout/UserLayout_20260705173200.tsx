...
const UserLayout: React.FC<UserLayoutProps> = ({ children, activeTab = "wallet", userData }) => {
  ...
  // ── DYNAMIC ROLE-BASED THEME ACCENTS ─────────────────────────────────────────
  const isDriver = userData?.role === "driver";
  const isCoop = userData?.role === "cooperative";

  let themeAccent = dark ? "#3B82F6" : "#2563EB"; // Blue (Commuter Default)
  let themeAccent50 = dark ? "rgba(59, 130, 246, 0.1)" : "#EFF6FF";
  let themeAccentText = dark ? "#60A5FA" : "#1D4ED8";

  if (isDriver) {
    themeAccent = dark ? "#F59E0B" : "#D97706"; // Amber
    themeAccent50 = dark ? "rgba(245, 158, 11, 0.1)" : "#FFFBEB";
    themeAccentText = dark ? "#FCD34D" : "#B45309";
  } else if (isCoop) {
    themeAccent = dark ? "#8B5CF6" : "#6D28D9"; // Purple
    themeAccent50 = dark ? "rgba(139, 92, 246, 0.1)" : "#F5F3FF";
    themeAccentText = dark ? "#A78BFA" : "#5B21B6";
  }
  ...
  if (isDriver) {
    dynamicMobileNav.splice(2, 0, { key: "loans", label: "Driver", icon: <IconList />, href: "/user/loans" });
  } else if (isCoop) {
    dynamicMobileNav.splice(2, 0, { key: "coop-pool", label: "Pool", icon: <IconList />, href: "/user/coop-pool" });
  }
    ...
    return (
      <a
        ...
        style={{
          ...
          color: isActive ? themeAccentText : (isHovered ? t.textPrim : t.textMuted),
          background: isActive ? themeAccent50 : (isHovered ? (dark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)") : "transparent"),
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <a href="/user" style={{ display: "flex", alignItems: "center", gap: 12, textDecoration: "none" }}>
              ...
              <div style={{ width: 40, height: 40, borderRadius: 12, background: themeAccent, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 4px 12px ${themeAccent}40` }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div ref={dropdownRef} style={{ position: "relative" }}>
              <button onClick={() => setDropdownOpen((v) => !v)} style={{ display: "flex", alignItems: "center", gap: 10, background: "transparent", border: `1px solid ${t.border}`, borderRadius: 999, padding: "4px 12px 4px 4px", cursor: "pointer", color: t.textPrim, transition: "background 0.2s" }} onMouseEnter={(e) => e.currentTarget.style.background = dark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)"} onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                {/* Dynamic User Avatar Background */}
                <div style={{ width: 32, height: 32, borderRadius: "50%", background: themeAccent, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                  {initials}
                </div>
                ...
                <div style={{ position: "absolute", top: "calc(100% + 12px)", right: 0, background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 16, minWidth: 240, boxShadow: dark ? "0 10px 40px rgba(0,0,0,0.5)" : "0 10px 40px -10px rgba(0,0,0,0.1)", zIndex: 200, overflow: "hidden", animation: "dropdownFade 0.2s ease-out" }}>
                  ...
                  <div style={{ borderTop: `1px solid ${t.border}`, padding: 8 }}>
                    <a href="/" style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", color: "#EF4444", textDecoration: "none", fontSize: 14, fontWeight: 600, borderRadius: 10, transition: "background 0.2s" }} onMouseEnter={(e) => e.currentTarget.style.background = dark ? "rgba(239, 68, 68, 0.1)" : "#FEF2F2"} onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                      <span style={{ color: "#EF4444" }}><IconLogout /></span> Sign out
                    </a>
                  </div>
                </div>
              )}
            ...
            <aside style={{ width: sidebarW, background: t.bgSidebar, borderRight: `1px solid ${t.border}`, display: "flex", flexDirection: "column", flexShrink: 0, transition: "transform .3s cubic-bezier(0.16, 1, 0.3, 1), margin .3s cubic-bezier(0.16, 1, 0.3, 1)", transform: sidebarOpen ? "translateX(0)" : `translateX(-${sidebarW}px)`, marginLeft: sidebarOpen ? 0 : -sidebarW, position: "sticky", top: 72, height: "calc(100vh - 72px)" }}>
              ...
              <div style={{ marginTop: "auto", padding: 20, borderTop: `1px solid ${t.border}`, background: dark ? "rgba(255,255,255,0.01)" : "#F8FAFC" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ width: 42, height: 42, borderRadius: 12, background: `linear-gradient(135deg, ${themeAccent}, ${dark ? "#1E293B" : "#94A3B8"})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 700, color: "#fff", flexShrink: 0, boxShadow: "0 4px 10px rgba(0,0,0,0.1)" }}>
                    {initials}
                  </div>
        ...
        {isMobile && (
          <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, height: 80, background: t.bgHeader, borderTop: `1px solid ${t.border}`, display: "flex", justifyContent: "space-around", alignItems: "center", zIndex: 100, paddingBottom: "env(safe-area-inset-bottom)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)" }}>
            {dynamicMobileNav.map((item) => {
              const isActive = activeTab === item.key;
              return (
                <a key={item.key} href={item.href} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, color: isActive ? themeAccentText : t.textMuted, textDecoration: "none", fontSize: 11, fontWeight: isActive ? 700 : 500, flex: 1, height: "100%", position: "relative" }}>
                  {isActive && <div style={{ position: "absolute", top: 0, width: 32, height: 3, background: themeAccentText, borderBottomLeftRadius: 4, borderBottomRightRadius: 4 }} />}
