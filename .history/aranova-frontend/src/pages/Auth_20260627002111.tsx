...
const AuthPage: React.FC = () => {
  ...
  // Mocks a 12 word phrase generation
  ...
  const validate = () => {
    ...
    if (authSuccess) {
      ...
      if (walletMode === "create") {
        if (!keysSaved) e.wallet = "You must confirm you have saved your recovery phrase securely.";
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (authSuccess) {
        ...
        if (user) {
          ...
          if (walletMode === "create" && generatedKeys) {
            // ENCRYPT: We use CryptoJS to lock the secret key using the user's PIN
            // This is the ONLY thing sent to Firebase. The actual secret/phrase stays local.
            ...
            // In a real app, you would use bip39 and ed25519-hd-key to derive the actual Keypair from the 12 words.
            // For this UI mockup, we simulate standard recovery by issuing a new pair and locking it with the NEW pin.
        ...
        if (form.email.includes("admin") || role === "cooperative") navigate("/admin");
        else navigate("/user");
        ...
        const userData: any = {
          ...
          role: role, approved: role === "commuter", walletCreated: false, createdAt: new Date().toISOString(),
        };
      ...
      setErrors({ general: err.code === "auth/email-already-in-use" ? "Email is already in use." : "An error occurred. Please try again." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const switchMode = (m: AuthMode) => {
    ...
    setWalletPin(""); setConfirmPin(""); setRecoveryPhrase("");
  };
                ...
                <>
                  <div style={{ marginBottom: "1rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                    <div>
                      ...
                      <select style={{ ...S.select, borderColor: errors.vehicleType ? "#EF4444" : "#E5E7EB" }} value={form.vehicleType} onChange={(e) => setForm({ ...form, vehicleType: e.target.value })}>
                        <option value="" disabled>Select...</option><option value="tricycle">Tricycle</option><option value="jeepney">Jeepney</option><option value="taxi">Taxi</option>
                      </select>
                    </div>
                    <div>
                  ...
                  <div style={{ marginBottom: "1.5rem" }}>
                    ...
                    <select style={{ ...S.select, borderColor: errors.selectedCoop ? "#EF4444" : "#E5E7EB" }} value={form.selectedCoop} onChange={(e) => setForm({ ...form, selectedCoop: e.target.value })}>
                      ...
                      {MOCK_COOPS.map((coop) => (<option key={coop.id} value={coop.id}>{coop.name}</option>))}
                    </select>
                  </div>
                </>
              )}
          ...
          {authSuccess && (
            <>
              <div style={{ display: "flex", gap: "10px", marginBottom: "1.5rem", flexWrap: "wrap" }}>
                <button onClick={handleGenerateWallet} style={{ ...S.ghostBtn, background: walletMode === "create" ? "#1652C9" : "#fff", color: walletMode === "create" ? "#fff" : "#374151" }}>Generate New Wallet</button>
                <button onClick={() => { setWalletMode("restore"); setGeneratedKeys(null); }} style={{ ...S.ghostBtn, background: walletMode === "restore" ? "#1652C9" : "#fff", color: walletMode === "restore" ? "#fff" : "#374151" }}>Restore Wallet</button>
              </div>
              ...
              {walletMode === "create" && generatedKeys && (
                <div style={{ background: "#F3F4F6", borderRadius: 12, padding: "20px", marginBottom: "1.5rem" }}>
                  ...
                  <label style={{ display: "flex", gap: 10, alignItems: "flex-start", fontSize: 13, color: "#111827", cursor: "pointer", fontWeight: 600 }}>
                    <input type="checkbox" checked={keysSaved} onChange={(e) => setKeysSaved(e.target.checked)} style={{ marginTop: 2, accentColor: "#1652C9" }} />
                    <span>I have securely written down my 12-word phrase.</span>
                  </label>
