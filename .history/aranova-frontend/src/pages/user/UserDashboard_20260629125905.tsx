...
const CONTRACT_ID = "CBKODGTL4BEG65F7IZZ2MWV3DNKXJ5LJJIA77TNIYSDL6N2BLDQOXGTO"; // Connected Smart Contract
  ...
  const handleRepayFuelCredit = async () => {
    ...
    try {
      ...
      const txBuilder = new TransactionBuilder(account, {
        ...
        networkPassphrase: NETWORK_PASSPHASE
      }).addOperation(Operation.invokeHostFunction({
        func: "repay_fuel_credit",
        args: [
          new Address(userData.publicKey),
          new Address("YOUR_TOKEN_CONTRACT_ADDRESS") // Replace with actual token
        ],
        auth: []
      }));

      await signAndSubmitStellarTransaction(txBuilder, userData, horizon);
      alert("Repayment successful! 0.5% service fee applied automatically.");
    } catch (err: any) {
...
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  // Replaced individual ceiling with a strict Global Pool state
  ...
  const handleUpdateSmartContractPool = async () => {
    ...
    try {
      ...
      // Instead of rigidly capping drivers, we deposit/bind the pool for the fleet.
      // (Optional native Stellar fallback for distributions if Contract isn't fully linked yet)
