...
// ---------------------------------------------------------------------------
// 1. DRIVER DASHBOARD (AUTOMATED DRIVER-INITIATED CREDIT CLAIM)
// ---------------------------------------------------------------------------
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const handleRequestFuelAdvanceAutomated = async () => {
    ...
    if (requestedAmt > coopCeiling) return alert(`Transaction Rejected: Requested amount exceeds your cooperative's threshold limit (${coopCeiling} XLM).`);

    setIsProcessing(true);
    try {
      // SECURE AUTOMATION LINK:
      // Hits your secure backend API node where the Coop key remains completely isolated from client inspection.
      ...
      if (!res.ok) {
        throw new Error("Backend server policy or limits validation failed.");
      }

      alert(`Fuel Credit Approved Automatically! ${requestedAmt} XLM has been pushed to your wallet.`);
      setFuelAllocationInput("");
    } catch (err: any) {
      alert("Instant routing pipeline error: " + err.message);
