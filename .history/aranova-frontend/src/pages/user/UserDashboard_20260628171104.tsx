...
import UserLayout, { useTheme } from "../../components/layout/UserLayout"; // NOTE: Fixed typo 'layouT' -> 'layout'
...
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const fetchDrivers = async () => {
    try {
      ...
      // Fetch Approved (Live Data Override)
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      setApprovedDriversCount(prev => prev + 1); // Live override on UI
    } catch (error) {
