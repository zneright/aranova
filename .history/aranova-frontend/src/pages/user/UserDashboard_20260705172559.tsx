...
import CommuterDashboardUI from './layouts/CommuterDashboardUI';
import DriverDashboardUI from './layouts/DriverDashboardUI';
import CoopDashboardUI from './layouts/CoopDashboardUI';
const { user, role, activities, vaultBalance, handleAction, loading } = useDashboardLogic();

// 2. Package the exact same data to pass down
const presentationalProps = { user, activities, vaultBalance, handleAction, loading };

// 3. Render purely visual, different layouts based on role
return (
    <div className={`theme-${role.toLowerCase()} min-h-screen w-full`}>
        {role === 'Commuter' && <CommuterDashboardUI {...presentationalProps} />}
        {role === 'Driver' && <DriverDashboardUI {...presentationalProps} />}
        {role === 'Cooperative' && <CoopDashboardUI {...presentationalProps} />}
    </div>
);
...
const CommuterPanel: React.FC<{ userData: any; onRefresh: () => void }> = ({ userData, onRefresh }) => {
    ...
    return (
        <>
            {/* INJECT UI STYLES FOR TRUE MOBILE-FIRST BENTO GRID & ANIMATIONS */}
            <style>{`
        /* Mobile-First Grid Layout */
        .bento-grid { 
          display: grid; 
          gap: 12px; 
          grid-template-columns: repeat(2, 1fr); /* 2 columns for secondary stats */
          margin-bottom: 24px; 
        }
        
        .bento-hero { 
          grid-column: span 2; /* Hero spans full width on mobile */
          display: flex; 
          flex-direction: column; 
          justify-content: center; 
          text-align: center; /* Centered looks cleaner on phones */
          padding: 24px 16px;
        }

        .bento-stat {
          grid-column: span 1; /* Stats side-by-side on mobile */
          padding: 16px;
          display: flex;
          flex-direction: column;
        ...
        @media (min-width: 768px) { 
          .bento-grid { gap: 20px; grid-template-columns: repeat(3, 1fr); }
          .bento-hero { grid-column: span 3; text-align: left; padding: 32px; align-items: flex-start; }
          .bento-stat { grid-column: span 1; padding: 24px; }
        }
        @media (min-width: 1024px) { 
          .bento-hero { grid-column: span 1; } /* Richer desktop 3-card row */
        }
        
        /* Card Surfaces - Tighter padding on mobile */
        .card-surface { 
          background: ${dark ? "#11131C" : "#FFFFFF"}; 
          border: 1px solid ${dark ? "#24283B" : "#E5E7EB"}; 
          border-radius: 20px; 
          box-shadow: ${dark ? "0 8px 24px -8px rgba(0,0,0,0.5)" : "0 4px 16px -4px rgba(0,0,0,0.05)"}; 
          transition: transform 0.2s cubic-bezier(0.16, 1, 0.3, 1), box-shadow 0.2s cubic-bezier(0.16, 1, 0.3, 1);
        }
        .card-surface:hover { 
          transform: translateY(-2px); 
          box-shadow: ${dark ? "0 12px 28px -8px rgba(0,0,0,0.6)" : "0 8px 24px -6px rgba(0,0,0,0.08)"}; 
        }
        ...
        /* Form & Actions - Thumb Friendly */
        .input-row { display: flex; flex-direction: column; gap: 12px; }
        @media (min-width: 768px) { .input-row { flex-direction: row; } }
        
        .action-grid {
          display: grid;
          grid-template-columns: repeat(2, 1fr); /* 2x2 grid for chunky mobile buttons */
          gap: 10px;
          margin-top: 12px;
        }
        @media (min-width: 768px) {
          .action-grid { display: flex; flex-direction: row; flex-wrap: wrap; }
          .action-grid > button { flex: 1; }
        }
        
        .btn-modern { 
          border: none; border-radius: 14px; padding: 14px 16px; font-weight: 700; font-size: 14px;
      ...
      `}</style>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                ...
                {/* Bento Grid Stats - Mobile First Layout */}
                <div className="bento-grid">
                    {/* Main Wallet Hero Card */}
                    <div className="card-surface card-hero-surface bento-hero">
                        <div style={{ fontSize: 12, fontWeight: 700, opacity: 0.8, textTransform: "uppercase", letterSpacing: "0.5px" }}>Available Balance</div>
                        {/* Using fluid typography (clamp) so it looks great on tiny phones and big monitors */}
                        <div style={{ fontSize: "clamp(32px, 8vw, 40px)", fontWeight: 800, marginTop: 4, marginBottom: 4, letterSpacing: "-1px" }}>{formatXlm(Number(userData.walletBalance || 0))} XLM</div>
                        <div style={{ fontSize: 13, opacity: 0.8 }}>Ready to send & receive</div>
                    </div>

                    <div className="card-surface bento-stat">
                        <div>
                            <div style={{ fontSize: 11, fontWeight: 700, color: dark ? "#94A3B8" : "#64748B", textTransform: "uppercase", letterSpacing: "0.5px" }}>Vault Staked</div>
                            <div style={{ fontSize: "clamp(20px, 5vw, 28px)", fontWeight: 800, marginTop: 8 }}>{formatXlm(Number(userData.vaultBalance || 0))}</div>
                            <div style={{ fontSize: 13, fontWeight: 700, marginTop: 2 }}>XLM</div>
                        </div>
                        <div style={{ fontSize: 11, color: dark ? "#64748B" : "#94A3B8", marginTop: 12, lineHeight: 1.4 }}>Earning yield via Soroban</div>
                    </div>

                    <div className="card-surface bento-stat">
                        <div>
                            <div style={{ fontSize: 11, fontWeight: 700, color: dark ? "#94A3B8" : "#64748B", textTransform: "uppercase", letterSpacing: "0.5px" }}>Trust Score</div>
                            <div style={{ fontSize: "clamp(20px, 5vw, 28px)", fontWeight: 800, marginTop: 8, color: dark ? "#34D399" : "#059669" }}>{Number(userData.trustScore || 0)} <span style={{ fontSize: 14, opacity: 0.5 }}>/100</span></div>
                        </div>
                        <div style={{ fontSize: 11, color: dark ? "#64748B" : "#94A3B8", marginTop: 12, lineHeight: 1.4 }}>Credit: {formatXlm(availableCredit)} XLM</div>
                    </div>
                </div>

                {/* Action Center Container */}
                <div className="card-surface" style={{ padding: "clamp(20px, 5vw, 32px)" }}>
                    <h3 style={{ marginTop: 0, marginBottom: 20, fontSize: 18, fontWeight: 800 }}>Quick Actions</h3>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                        ...
                        {/* Action Buttons Row - Thumb Friendly Grid */}
                        <div className="action-grid">
                            <button className="btn-modern btn-primary" onClick={handleSend} disabled={busy}>
                                {busy ? "Processing..." : "Send XLM"}
                            </button>
                            <button className="btn-modern btn-ghost" onClick={() => setShowReceiveModal(true)}>
                                Receive
                            </button>
                            <button className="btn-modern btn-ghost" onClick={() => setScanning((s) => !s)}>
                                {scanning ? "Close Cam" : "📷 Scan QR"}
                            </button>
                            <button className="btn-modern btn-ghost" style={{ opacity: 0.6 }} onClick={() => alert("Withdraw is reserved for future PDAX integration.")}>
                                Withdraw
                            </button>
                        </div>
                    </div>
                ...
                {scanning && (
                    <div className="card-surface" style={{ textAlign: "center", padding: "clamp(20px, 5vw, 32px)", animation: "fade 0.3s" }}>
                ...
                {showReceiveModal && (
                    <div className="modal-overlay" onClick={() => setShowReceiveModal(false)}>
                        <div className="modal-content" onClick={e => e.stopPropagation()} style={{ textAlign: "center", padding: "clamp(24px, 6vw, 32px)" }}>
                ...
                {activeReceipt && (
                    <div className="modal-overlay">
                        <div className="modal-content" style={{ padding: "clamp(24px, 6vw, 32px)" }}>
                            <div style={{ textAlign: "center", marginBottom: 28 }}>
                                ...
                                <div style={{ color: dark ? "#34D399" : "#059669", fontWeight: 900, fontSize: "clamp(28px, 8vw, 36px)", letterSpacing: "-1px" }}>{activeReceipt.amount} XLM</div>
