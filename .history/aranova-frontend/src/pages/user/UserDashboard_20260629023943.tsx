    ...
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="dash-grid">
        <div className="dash-col-main" style={{ gridColumn: "1 / 4", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div onClick={handlePayFare} style={{ background: "#ECFDF5", border: "1px solid #6EE7B7", borderRadius: 20, padding: 22, display: "flex", flexDirection: "column", alignItems: "center", gap: 10, cursor: isProcessing ? "not-allowed" : "pointer", opacity: isProcessing ? 0.6 : 1 }}>
              <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#D1FAE5", color: "#059669", display: "flex", alignItems: "center", justifyContent: "center" }}><IconSend /></div>
              <span style={{ fontWeight: 800, fontSize: 15, color: "#059669" }}>{isProcessing ? "Sending..." : "Pay Fare"}</span>
            </div>
            <div onClick={() => alert(`Your Public Key for receiving funds is:\n\n${userData.publicKey}`)} style={{ background: "#FFFBEB", border: "1px solid #F4D03F", borderRadius: 20, padding: 22, display: "flex", flexDirection: "column", alignItems: "center", gap: 10, cursor: "pointer" }}>
              <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#FEF9C3", color: "#92400E", display: "flex", alignItems: "center", justifyContent: "center" }}><IconScan /></div>
              <span style={{ fontWeight: 800, fontSize: 15, color: "#111827" }}>Receive Funds</span>
            </div>
          </div>
        </div>
      </div>
...
// ---------------------------------------------------------------------------
// 3. COOPERATIVE DASHBOARD (GLOBAL PRE-APPROVED CEILING MANAGER)
// ---------------------------------------------------------------------------
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  const { dark } = useTheme();
  const [approvedDrivers, setApprovedDrivers] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [globalPoolAmount, setGlobalPoolAmount] = useState<string>(() => localStorage.getItem(`aranova_offline_limit_${userData?.uid}`) || "100");
  const [liveRate, setLiveRate] = useState<number>(7.65);
  const [isProcessing, setIsProcessing] = useState(false);
  ...
  const fetchDrivers = async () => {
    try {
      ...
      if (coopDoc.exists()) {
        const val = String(coopDoc.data().fuelCreditLimit || "100");
        setGlobalPoolAmount(val);
  ...
  const handleUpdateGlobalFleetLimit = async () => {
    ...
    const conf = window.confirm(`Deploy global on-chain budget matrix allocation of ${globalPoolAmount} XLM distributed among all approved fleet keys?`);
    if (!conf) return;
    ...
    try {
      ...
      // Setup clean types matching the latest @stellar/stellar-sdk version
      const txBuilder = new TransactionBuilder(account, {
        fee: fee.toString(),
        networkPassphrase: NETWORK_PASSPHRASE
      });

      // Distribute claim entries evenly across overall active system drivers automatically
      const splittingWeight = (Number(globalPoolAmount) / approvedDrivers.length).toFixed(4);

      approvedDrivers.forEach((driver) => {
        txBuilder.addOperation(Operation.createClaimableBalance({
          claimants: [
            { destination: driver.publicKey, predicate: Horizon.Predicate.unconditional() }
          ],
          asset: Asset.native(),
          amount: splittingWeight
        }));
      });
      ...
      // Sync limits to Firestore cache layer
      await setDoc(doc(db, "cooperatives", userData.uid), {
        fuelCreditLimit: Number(globalPoolAmount),
        publicKey: userData.publicKey,
        updatedAt: new Date().toISOString()
      }, { merge: true });

      localStorage.setItem(`aranova_offline_limit_${userData.uid}`, globalPoolAmount);
      alert("On-chain Fleet Budget matrix allocated and synced successfully!");
    } catch (err: any) {
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      {/* OVERALL DRIVERS PRE-APPROVED BUDGET POOL PARAMETERS SETUP */}
      <div style={{ background: dark ? "#3B210B" : "#FFFBEB", border: `1px solid ${dark ? "#78350F" : "#FDE68A"}`, borderRadius: 16, padding: "16px 20px", marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontWeight: 800, fontSize: 14, color: dark ? "#FCD34D" : "#92400E" }}>Overall Fleet Budget Ceiling</div>
          <div style={{ fontSize: 12, color: dark ? "#D97706" : "#B45309", marginTop: 4 }}>Inject custom pool sizing parameters to distribute equal share slices across active drivers. Value: ≈ ₱{(coopLimitInput * liveRate).toFixed(2)} PHP</div>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <input type="number" value={globalPoolAmount} onChange={e => setGlobalPoolAmount(e.target.value)} style={{ width: 90, padding: "8px", borderRadius: 8, border: `1px solid ${dark ? "#78350F" : "#FDE68A"}`, background: dark ? "#451A03" : "#fff", color: t.textPrim, fontWeight: 700 }} />
          <button onClick={handleUpdateGlobalFleetLimit} disabled={isProcessing} style={{ background: "#F59E0B", color: "#fff", border: "none", padding: "8px 16px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", opacity: isProcessing ? 0.6 : 1 }}>
            {isProcessing ? "Syncing Pool..." : "Fund Fleet Matrix"}
          </button>
        </div>
      </div>
