...
const submitRealSorobanTransaction = async (publicKey: string, method: string, args: any[]) => {
  if (!(await isConnected())) throw new Error("Freighter wallet is not connected.");

  // Use SorobanRpc.Server directly from the import
  ...
  // Use the correct check for simulation errors
  ...
  if (sendResponse.status === "ERROR") throw new Error("Transaction submission rejected.");
...
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const executeFuelCreditContract = async () => {
    ...
    try {
      ...
      const args = [
        new StellarSdk.Address(userData.publicKey).toScVal(),
        new StellarSdk.Address(coopWalletData).toScVal(),
        new StellarSdk.Address(PARTNER_PUMP_PUBLIC_KEY).toScVal(),
      ...
      alert("Smart contract execution failed: " + error.message);
    }
