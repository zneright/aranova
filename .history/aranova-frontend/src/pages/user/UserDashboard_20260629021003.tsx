...
// ---------------------------------------------------------------------------
// Global Constant Layer Config (SINGLE CONTRACT AS REQUESTED)
// ---------------------------------------------------------------------------
const ARANOVA_CONTRACT_ID = "CAGIHBOLPSZX5MKWGGEEGJ323WWDVVG2CHXWQOV4VKGTZL2U6FY7NANH";
...
const fundWithFriendbot = async (publicKey: string) => {
  console.log("Triggering Friendbot to fund account on Testnet...");
  try {
...
// ---------------------------------------------------------------------------
const submitRealSorobanTransaction = async (publicKey: string, method: string, args: xdr.ScVal[]) => {
  const server = new rpc.Server(SOROBAN_RPC_URL);
  let account;

  // 1. Fetch Account & Auto-Fund if it doesn't exist
  try {
    account = await server.getAccount(publicKey);
  } catch (err: any) {
    if (err.message && err.message.toLowerCase().includes("not found")) {
      const funded = await fundWithFriendbot(publicKey);
      if (!funded) throw new Error("Account not found and Friendbot funding failed. Please fund manually.");
      // Wait a moment for network to register the funding
  ...
  let tx = new TransactionBuilder(account, {
    fee: "100000", // Bumped fee to ensure it processes
  ...
  let assembledTx = rpc.assembleTransaction(tx, simulated);
  let finalTx = typeof (assembledTx as any).build === 'function' ? (assembledTx as any).build() : assembledTx;

  // 5. Sign Transaction
  const walletModule = await getActiveWalletModule();
  const signResult: any = await walletModule.signTransaction(finalTx.toXDR(), { networkPassphrase: NETWORK_PASSPHRASE, network: "TESTNET" });

  const signedXdr = typeof signResult === 'string' ? signResult : (signResult.signedXDR || signResult.signedTxXdr);
  if (!signedXdr) throw new Error("Failed to extract signed XDR from wallet response.");

  const signedTx = TransactionBuilder.fromXDR(signedXdr, NETWORK_PASSPHRASE);

  // 6. Send to Network
  console.log(`Sending ${method} to network...`);
  ...
  const handleUpdateVault = async () => {
    try {
      const args = [
        ...
        nativeToScVal(savePercentage * 100, { type: 'i128' }),
...
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const executeFuelCreditContract = async () => {
    ...
    if (requestedAmt > coopCeiling) return alert(`Transaction Rejected: Capital requested (${requestedAmt} XLM) triggers a policy breach against your Cooperative limit (${coopCeiling} XLM).`);
    if (!coopWalletData) return alert("Cooperative wallet not found. Ask Admin to link their Soroban ID.");

    setIsProcessingContract(true);
    try {
      // ── SINGLE CONTRACT CALL ──
      const args = [
        new Address(userData.publicKey).toScVal(), // driver
        new Address(coopWalletData).toScVal(),     // coop_wallet
        new Address(ARANOVA_CONTRACT_ID).toScVal(), // token_addr (reusing single contract ID)
        nativeToScVal(requestedAmt * 10000000, { type: 'i128' }) // convert to stroops
      ];

      await submitRealSorobanTransaction(userData.publicKey, "issue_fuel_credit", args);

      alert(`Fuel Credit Approved! ${requestedAmt} XLM automatically routed from Cooperative into your wallet. Use 'Send Funds' to pay the gas station.`);
      setCustomRequestAmount("");
      setBalance(prev => (parseFloat(prev) + requestedAmt).toFixed(2));
    } catch (error: any) {
      alert("Fuel advance failed: " + error.message);
  ...
  const executeRepayCreditContract = async () => {
    ...
    try {
      ...
      const args = [
        new Address(userData.publicKey).toScVal(),
        new Address(ARANOVA_CONTRACT_ID).toScVal()
      ];
      ...
      const res = await fetch(`https://horizon-testnet.stellar.org/accounts/${userData.publicKey}`);
      const data = await res.json();
      if (data.balances) {
        const xlm = data.balances.find((b: any) => b.asset_type === 'native');
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="dash-grid">
        <div className="dash-col-main" style={{ gridColumn: "1 / 3", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            ...
            <div onClick={() => alert("Initiating outbound transaction to pay Gas Station or Store...")} style={{ background: dark ? "#064E3B" : "#ECFDF5", border: `1px solid ${dark ? "#047857" : "#6EE7B7"}`, borderRadius: 20, padding: 22, display: "flex", flexDirection: "column", alignItems: "center", gap: 10, cursor: "pointer" }}>
              ...
              <span style={{ fontSize: 11, color: t.textMuted, textAlign: "center" }}>Pay gas stations or stores</span>
            </div>
          </div>
        </div>
        ...
        <div className="dash-col-side" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderLeft: "4px solid #F59E0B", borderRadius: "0 20px 20px 0", padding: 20 }}>
            ...
            <button onClick={executeFuelCreditContract} disabled={isProcessingContract} style={{ width: "100%", background: dark ? "#451A03" : "#FEF3C7", color: dark ? "#FCD34D" : "#92400E", border: "none", borderRadius: 8, padding: "10px", fontWeight: 700, fontSize: 12, cursor: "pointer", opacity: isProcessingContract ? 0.6 : 1, marginBottom: 8 }}>
              {isProcessingContract ? "Processing Ledger..." : "Request Fuel Advance"}
            </button>
...
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const handleUpdateLimit = async () => {
    try {
      ...
      // SINGLE CONTRACT CALL
      const args = [
        new Address(userData.publicKey).toScVal(), // Coop
        new Address(userData.publicKey).toScVal(), // Driver Placeholder to set global variable
        nativeToScVal(coopLimitInput, { type: 'i128' })
      ];
      ...
      alert("Contract limit failed: " + err.message);
    } finally {
    ...
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      <div style={{ background: dark ? "#3B210B" : "#FFFBEB", border: `1px solid ${dark ? "#78350F" : "#FDE68A"}`, borderRadius: 16, padding: "16px 20px", marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button onClick={handleUpdateLimit} disabled={isProcessing} style={{ background: "#F59E0B", color: "#fff", border: "none", padding: "8px 16px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", opacity: isProcessing ? 0.6 : 1 }}>
