...
// ─── REAL BLOCKCHAIN IMPORTS (FIXED TS ERRORS) ───
import {
  Networks,
  TransactionBuilder,
  Contract,
...
// Direct module imports (bypassing the wrapper to prevent WalletNetwork TS errors)
...
const XLM_NATIVE_CONTRACT_ID = "CAGIHBOLPSZX5MKWGGEEGJ323WWDVVG2CHXWQOV4VKGTZL2U6FY7NANH"; // Testnet XLM
...
// ---------------------------------------------------------------------------
// HELPER: DYNAMIC WALLET DETECTOR
// ---------------------------------------------------------------------------
const getActiveWalletModule = async (publicKey: string) => {
  ...
  for (const mod of modules) {
    if (await mod.isAvailable()) {
      try {
        ...
        if (res.address === publicKey) return mod;
      } catch (e) { /* ignore and check next */ }
    }
  }
  // Fallback if exactly matching key isn't found (e.g., wallet locked)
  const freighter = new FreighterModule();
  if (await freighter.isAvailable()) return freighter;
...
const submitRealSorobanTransaction = async (publicKey: string, method: string, args: xdr.ScVal[]) => {
  ...
  // 2. Assemble resources (Fixed TransactionBuilder conversion error)
  ...
  // 3. Dynamically sign with the correct wallet extension
  const walletModule = await getActiveWalletModule(publicKey);
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 20 }}>
            {wallets.map(w => (
              <div key={w.address} style={{ background: t.bg, borderRadius: 12, padding: 14, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexShrink: 0 }}> {/* Fixed React Style Error */}
...
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const executeFuelCreditContract = async () => {
    ...
    try {
      // ── REAL SOROBAN CALL: issue_fuel_credit (Coop -> Driver) ──
      const args = [
        new Address(userData.publicKey).toScVal(), // driver
        new Address(coopWalletData).toScVal(),     // coop_wallet
        new Address(XLM_NATIVE_CONTRACT_ID).toScVal(), // token_addr
