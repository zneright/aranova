...
// ---------------------------------------------------------------------------
// HELPER: EXCHANGE RATES (WITH OFFLINE SIMULATION RETENTION)
// ---------------------------------------------------------------------------
  ...
  const fetchBalance = () => {
    ...
    fetch(`${HORIZON_RPC_URL}/accounts/${userData.publicKey}`)
      ...
      .then(data => {
        ...
        // Safe degraded state retainment from storage if connection times out
...
// ---------------------------------------------------------------------------
// 1. DRIVER DASHBOARD (INSTANT RECLAIM PIPELINE)
// ---------------------------------------------------------------------------
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  // Offline-first ceiling fetch
  const [coopCeiling, setCoopCeiling] = useState<number>(() =>
    Number(localStorage.getItem(`aranova_offline_limit_${userData?.uid}`)) || 0
  );
  ...
  // Fetch Cooperative Rules with Offline Cache (Prioritizing Per-Driver Limit)
  useEffect(() => {
    // Check if the driver has a specific ceiling set on their own document first
    if (userData?.fuelCreditLimit !== undefined) {
      setCoopCeiling(userData.fuelCreditLimit);
      localStorage.setItem(`aranova_offline_limit_${userData.uid}`, String(userData.fuelCreditLimit));
    } else if (userData?.cooperativeId) {
      getDoc(doc(db, "cooperatives", userData.cooperativeId)).then(docSnap => {
        if (docSnap.exists()) {
          const data = docSnap.data();
          const ceiling = data.fuelCreditLimit || 0;
          setCoopCeiling(ceiling);
          localStorage.setItem(`aranova_offline_limit_${userData.uid}`, String(ceiling));
        }
      }).catch(err => {
        console.warn("Offline degraded mode. Using cached limit.", err);
        const cached = localStorage.getItem(`aranova_offline_limit_${userData.uid}`);
        if (cached) setCoopCeiling(Number(cached));
      });
    }
  }, [userData]);
  ...
  const handleClaimAllocationOnChain = async () => {
    ...
    try {
      ...
      alert(`Success! Instantly claimed ${allocationAmount} XLM directly from the Cooperative allocation layer.`);
      setPendingAllocationId(null);
      setAllocationAmount("0.00");

      // Manually trigger a UI refresh of the balance
      ...
      alert("Claim signature execution failed: " + err.message);
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="dash-grid">
        ...
        <div className="dash-col-side" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderLeft: "4px solid #F59E0B", borderRadius: "0 20px 20px 0", padding: 20 }}>
            <div style={{ fontWeight: 800, fontSize: 14, color: t.textPrim, display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}><IconFuel /> Automated Fuel Credit</div>

            <p style={{ fontSize: 11, color: t.textMuted, margin: "0 0 12px" }}>Fleet Operational Ceiling: {coopCeiling} XLM (≈ ₱{(coopCeiling * liveRate).toFixed(2)} PHP)</p>
            ...
            {pendingAllocationId ? (
              <div style={{ marginTop: 12 }}>
                ...
                <button onClick={handleClaimAllocationOnChain} disabled={isProcessing} style={{ width: "100%", background: "#10B981", color: "#fff", border: "none", borderRadius: 8, padding: "12px", fontWeight: 800, fontSize: 13, cursor: "pointer", opacity: isProcessing ? 0.6 : 1 }}>
                  {isProcessing ? "Signing Protocol Execution..." : "Instantly Liquidate Allocation"}
                </button>
              </div>
            ) : (
              <p style={{ fontSize: 12, color: t.textMuted, lineHeight: 1.5, margin: "10px 0 0" }}>
                No fresh pre-authorized allocation detected on-chain. When the cooperative funds your fleet tier, your automated voucher appears here.
              </p>
            )}
          </div>
        </div>
...
// ---------------------------------------------------------------------------
// 3. COOPERATIVE DASHBOARD (PER-DRIVER CEILING MANAGER)
// ---------------------------------------------------------------------------
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  const { dark } = useTheme();
  const [approvedDrivers, setApprovedDrivers] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [perDriverCeiling, setPerDriverCeiling] = useState<string>(() => localStorage.getItem(`aranova_offline_limit_${userData?.uid}`) || "10");
  const [liveRate, setLiveRate] = useState<number>(7.65);
  const [isProcessing, setIsProcessing] = useState(false);
  ...
  const fetchDrivers = async () => {
    try {
      ...
      getDoc(doc(db, "cooperatives", userData.uid)).then(coopDoc => {
        if (coopDoc.exists()) {
          const val = String(coopDoc.data().fuelCreditLimit || "10");
          setPerDriverCeiling(val);
          localStorage.setItem(`aranova_offline_limit_${userData.uid}`, val);
        }
      }).catch(err => {
        console.warn("Offline mode - using cached coop pool value.", err);
        const cached = localStorage.getItem(`aranova_offline_limit_${userData.uid}`);
        if (cached) setPerDriverCeiling(cached);
      });
  ...
  const handleUpdateFleetLimits = async () => {
    ...
    const limitNum = Number(perDriverCeiling);
    if (isNaN(limitNum) || limitNum <= 0) return alert("Invalid ceiling amount.");

    const totalNeeded = limitNum * approvedDrivers.length;
    const conf = window.confirm(`Deploy on-chain allocation? This will distribute exactly ${limitNum} XLM to EACH of your ${approvedDrivers.length} approved drivers (Total transaction: ${totalNeeded} XLM).`);
    if (!conf) return;
    ...
    try {
      ...
      // Assign the exact ceiling amount per driver instead of dividing a pool
      const exactAllocation = limitNum.toFixed(4);

      approvedDrivers.forEach((driver) => {
        if (driver.publicKey) {
          txBuilder.addOperation(Operation.createClaimableBalance({
            claimants: [
              new Claimant(driver.publicKey, Claimant.predicateUnconditional())
            ],
            asset: Asset.native(),
            amount: exactAllocation
          }));
        }
      });
      ...
      // THE FIX: Remove `await` so Firestore background-syncs don't hang the UI if offline!
      setDoc(doc(db, "cooperatives", userData.uid), {
        fuelCreditLimit: limitNum,
        publicKey: userData.publicKey,
        updatedAt: new Date().toISOString()
      }, { merge: true }).catch(e => console.warn("Deferred coop sync", e));

      // Set the specific limit inside each driver's document
      approvedDrivers.forEach(driver => {
        updateDoc(doc(db, "users", driver.id), { fuelCreditLimit: limitNum })
          .catch(e => console.warn("Deferred driver sync", e));
      });

      localStorage.setItem(`aranova_offline_limit_${userData.uid}`, perDriverCeiling);
      alert(`Success! Per-Driver Ceiling set to ${limitNum} XLM and distributed on-chain.`);
    } catch (err: any) {
      alert("Matrix allocation build failed: " + err.message);
    } finally { setIsProcessing(false); }
  };
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      {/* PER-DRIVER CEILING PARAMETERS SETUP */}
      <div style={{ background: dark ? "#3B210B" : "#FFFBEB", border: `1px solid ${dark ? "#78350F" : "#FDE68A"}`, borderRadius: 16, padding: "16px 20px", marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontWeight: 800, fontSize: 14, color: dark ? "#FCD34D" : "#92400E" }}>Per-Driver Fuel Ceiling</div>
          <div style={{ fontSize: 12, color: dark ? "#D97706" : "#B45309", marginTop: 4 }}>Set the exact XLM limit allocated to EACH driver. Value per driver: ≈ ₱{(Number(perDriverCeiling) * liveRate).toFixed(2)} PHP</div>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <input type="number" value={perDriverCeiling} onChange={e => setPerDriverCeiling(e.target.value)} style={{ width: 90, padding: "8px", borderRadius: 8, border: `1px solid ${dark ? "#78350F" : "#FDE68A"}`, background: dark ? "#451A03" : "#fff", color: t.textPrim, fontWeight: 700 }} />
          <button onClick={handleUpdateFleetLimits} disabled={isProcessing} style={{ background: "#F59E0B", color: "#fff", border: "none", padding: "8px 16px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", opacity: isProcessing ? 0.6 : 1 }}>
            {isProcessing ? "Syncing Limits..." : "Fund Fleet Matrix"}
          </button>
        </div>
      </div>
      ...
      <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 20, padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h3 style={{ fontWeight: 800, fontSize: 16, color: t.textPrim }}>Active Fleet Fleet Status</h3>
