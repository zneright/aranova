...
const submitRealStellarPayment = async (senderPubKey: string, receiverPubKey: string, amountXlm: string) => {
  const horizon = new Server(HORIZON_RPC_URL);
  ...
  const signResult: any = await walletModule.signTransaction(tx.toXDR(), { networkPassphrase: NETWORK_PASSPHRASE, network: "TESTNET" });
...
// ---------------------------------------------------------------------------
const LiveWalletWidget: React.FC<{ userData: any, liveRate: number, dark: boolean }> = ({ userData, liveRate, dark }) => {
    ...
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="dash-grid">
        <div className="dash-col-main" style={{ gridColumn: "1 / 3", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <LiveWalletWidget userData={userData} liveRate={liveRate} dark={dark} />
...
// ---------------------------------------------------------------------------
// 3. COOPERATIVE DASHBOARD (NOW HAS FULL WALLET AND DIRECT SENDING)
// ---------------------------------------------------------------------------
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const handleSendFuelCredit = async (driver: any) => {
    ...
    setIsProcessing(true);
    try {
      // 100% Guaranteed On-Chain Transfer from Coop to Driver
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      {/* DRIVER MANAGEMENT & FUEL ISSUANCE */}
...
const UserDashboard: React.FC = () => {
  ...
  if (authLoading || userData === null) return <LoadingWorkspace message="Connecting to Stellar Horizon Node..." dark={dark} />;
