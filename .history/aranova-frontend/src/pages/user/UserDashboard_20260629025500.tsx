...
// ---------------------------------------------------------------------------
// 3. COOPERATIVE DASHBOARD (POOL + CEILING MANAGER)
// ---------------------------------------------------------------------------
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  // Two distinct controls for the new logic
  const [globalPoolAmount, setGlobalPoolAmount] = useState<string>(() => localStorage.getItem(`aranova_pool_${userData?.uid}`) || "500");
  const [driverCeiling, setDriverCeiling] = useState<string>(() => localStorage.getItem(`aranova_ceiling_${userData?.uid}`) || "50");
    ...
    try {
      ...
      getDoc(doc(db, "cooperatives", userData.uid)).then(coopDoc => {
        if (coopDoc.exists()) {
          ...
          if (data.fuelCreditLimit) setDriverCeiling(String(data.fuelCreditLimit));
        }
  ...
  // Fire-and-forget sync function (physically cannot block the UI thread)
  const syncToFirebaseBackground = (pool: number, ceiling: number, drivers: any[]) => {
    setDoc(doc(db, "cooperatives", userData.uid), {
      globalPool: pool,
      fuelCreditLimit: ceiling,
      publicKey: userData.publicKey,
      updatedAt: new Date().toISOString()
    }, { merge: true }).catch(() => { });

    drivers.forEach(driver => {
      updateDoc(doc(db, "users", driver.id), { fuelCreditLimit: ceiling }).catch(() => { });
    });
  };
  ...
  const handleUpdateFleetLimits = async () => {
    ...
    if (isNaN(ceilingNum) || ceilingNum <= 0) return alert("Invalid ceiling amount.");

    // MATH: Divide pool among drivers, but hard-cap it at the ceiling
    let calculatedAllocation = poolNum / approvedDrivers.length;
    ...
    const conf = window.confirm(`DEPLOYMENT REVIEW:\n\nGlobal Pool: ${poolNum} XLM\nMax Ceiling: ${ceilingNum} XLM per driver\n\nResult: Each of your ${approvedDrivers.length} drivers will receive exactly ${finalAllocationXLM} XLM.\nTotal checkout: ${actualTotalSpent} XLM.\n\nProceed to sign?`);
    if (!conf) return;
    ...
    try {
      ...
      // If user ignores the popup, this is where it usually gets stuck.
      const signResult: any = await walletModule.signTransaction(tx.toXDR(), { networkPassphrase: NETWORK_PASSPHRASE });
      const signedXdr = typeof signResult === 'string' ? signResult : (signResult.signedXDR || signResult.signedTxXdr);
      ...
      // Force a 25-second timeout on the Horizon submission so it never hangs infinitely
      ...
      // Background sync data (Unblocks UI)
      syncToFirebaseBackground(poolNum, ceilingNum, approvedDrivers);
      localStorage.setItem(`aranova_pool_${userData.uid}`, String(poolNum));
      localStorage.setItem(`aranova_ceiling_${userData.uid}`, String(ceilingNum));

      alert(`Success! ${finalAllocationXLM} XLM was routed to all approved drivers.`);
    } catch (err: any) {
      alert("Allocation Failed: " + (err.message || "User declined signature or network disconnected."));
    } finally {
      // GUARANTEED to execute and unfreeze the button
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      {/* POOL & CEILING METRICS CONTROLS */}
      <div style={{ background: dark ? "#3B210B" : "#FFFBEB", border: `1px solid ${dark ? "#78350F" : "#FDE68A"}`, borderRadius: 16, padding: "20px", marginBottom: 24 }}>
        <h3 style={{ margin: "0 0 16px", fontSize: 15, fontWeight: 800, color: dark ? "#FCD34D" : "#92400E" }}>Fleet Budget Allocation Matrix</h3>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 16 }}>
          ...
          <div>
            ...
            <span style={{ fontSize: 11, color: t.textMuted, display: "block", marginTop: 4 }}>≈ ₱{(Number(globalPoolAmount) * liveRate).toFixed(2)} PHP Total</span>
          </div>

          {/* Driver Ceiling */}
          <div>
            <label style={{ display: "block", fontSize: 12, fontWeight: 800, color: t.textMuted, marginBottom: 6 }}>MAX CEILING PER DRIVER (XLM)</label>
            <input type="number" value={driverCeiling} onChange={e => setDriverCeiling(e.target.value)} style={{ width: "100%", boxSizing: "border-box", padding: "10px", borderRadius: 8, border: `1px solid ${dark ? "#78350F" : "#FDE68A"}`, background: dark ? "#451A03" : "#fff", color: t.textPrim, fontWeight: 700 }} />
            <span style={{ fontSize: 11, color: t.textMuted, display: "block", marginTop: 4 }}>Hard cap per vehicle</span>
          </div>
        </div>

        <button onClick={handleUpdateFleetLimits} disabled={isProcessing} style={{ width: "100%", background: "#F59E0B", color: "#fff", border: "none", padding: "12px", borderRadius: 8, fontSize: 13, fontWeight: 800, cursor: isProcessing ? "not-allowed" : "pointer", opacity: isProcessing ? 0.6 : 1 }}>
          {isProcessing ? "Processing Signature & Awaiting Network..." : "Sign & Distribute Funds"}
        </button>
      </div>
