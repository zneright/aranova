...
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const [coopCeiling, setCoopCeiling] = useState<number>(() =>
    Number(localStorage.getItem(`aranova_offline_limit_${userData?.cooperativeId}`)) || 0
  );
  ...
  // Fetch Cooperative Rules with Offline Cache
  useEffect(() => {
    if (userData?.cooperativeId) {
      getDoc(doc(db, "cooperatives", userData.cooperativeId)).then(docSnap => {
        if (docSnap.exists()) {
          ...
          localStorage.setItem(`aranova_offline_limit_${userData.cooperativeId}`, String(ceiling));
        }
      }).catch(err => {
        console.warn("Offline degraded mode. Using cached limit.", err);
        const cached = localStorage.getItem(`aranova_offline_limit_${userData.cooperativeId}`);
...
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const fetchDrivers = async () => {
    try {
      ...
      getDoc(doc(db, "cooperatives", userData.uid)).then(coopDoc => {
        ...
        if (cached) setGlobalPoolAmount(cached);
      });
  ...
  const handleUpdateGlobalFleetLimit = async () => {
    ...
    try {
      ...
      // Distribute claim entries dynamically matching newest Claimant instantiations
    ...
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      <div style={{ background: dark ? "#3B210B" : "#FFFBEB", border: `1px solid ${dark ? "#78350F" : "#FDE68A"}`, borderRadius: 16, padding: "16px 20px", marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: dark ? "#D97706" : "#B45309", marginTop: 4 }}>Inject custom pool sizing parameters to distribute equal share slices across active drivers. Value: ≈ ₱{(Number(globalPoolAmount) * liveRate).toFixed(2)} PHP</div>
        </div>
