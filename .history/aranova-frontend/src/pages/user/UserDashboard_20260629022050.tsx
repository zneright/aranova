...
// ---------------------------------------------------------------------------
// 1. DRIVER DASHBOARD (AUTOMATED DRIVER-INITIATED FLOW)
// ---------------------------------------------------------------------------
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  const { dark } = useTheme();
  const [coopWalletData, setCoopWalletData] = useState<string>("");
  const [fuelAllocationInput, setFuelAllocationInput] = useState<string>("");
  ...
  const handleRequestFuelAdvanceAutomated = async () => {
    const requestedAmt = Number(fuelAllocationInput);
    if (!requestedAmt || requestedAmt <= 0) return alert("Please specify a valid amount of XLM.");
    if (requestedAmt > coopCeiling) return alert(`Transaction Rejected: Requested amount exceeds your cooperative's standard threshold limit (${coopCeiling} XLM).`);

    setIsProcessing(true);
    try {
      // Execute transaction. In staging/production, this triggers your backend endpoint 
      // containing the Cooperative's operational distribution credentials to sign automatically.
      const res = await fetch("https://your-firebase-cloud-functions-url/claimFuelAdvance", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          driverUid: userData.uid,
          driverPubKey: userData.publicKey,
          cooperativeId: userData.cooperativeId,
          amountXlm: requestedAmt
        })
      });

      if (!res.ok) {
        throw new Error("Automated policy verification or distribution sign request failed.");
      }

      alert(`Fuel Credit Approved Automatically! ${requestedAmt} XLM has been loaded to your wallet balance.`);
      setFuelAllocationInput("");
    } catch (err: any) {
      alert("Automated allocation run failed. Defaulting to fallback pipeline: " + err.message);
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="dash-grid">
        ...
        <div className="dash-col-side" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderLeft: "4px solid #F59E0B", borderRadius: "0 20px 20px 0", padding: 20 }}>
            <div style={{ fontWeight: 800, fontSize: 14, color: t.textPrim, display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}><IconFuel /> Claim Fuel Credit</div>
            <p style={{ fontSize: 11, color: t.textMuted, margin: "0 0 12px" }}>Daily limit: {coopCeiling} XLM (≈ ₱{(coopCeiling * liveRate).toFixed(2)} PHP)</p>

            <input
              type="number"
              placeholder="Enter XLM value..."
              value={fuelAllocationInput}
              onChange={e => setFuelAllocationInput(e.target.value)}
              style={{ width: "100%", padding: "10px", borderRadius: 8, border: `1px solid ${t.border}`, background: dark ? "#13151F" : "#F9FAFB", color: t.textPrim, fontWeight: 700, outline: "none", boxSizing: "border-box", marginBottom: 12 }}
            />

            <button onClick={handleRequestFuelAdvanceAutomated} disabled={isProcessing} style={{ width: "100%", background: dark ? "#451A03" : "#FEF3C7", color: dark ? "#FCD34D" : "#92400E", border: "none", borderRadius: 8, padding: "12px", fontWeight: 800, fontSize: 13, cursor: "pointer", opacity: isProcessing ? 0.6 : 1 }}>
              {isProcessing ? "Executing Instant Transfer..." : "Instantly Claim Fuel Advance"}
            </button>
          </div>
        </div>
      </div>
...
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const handleSendFuelCreditDirect = async (driver: any) => {
    ...
    const amt = window.prompt(`Enter amount of XLM to push to ${driver.displayName} for Manual Fuel Credit allocation:`);
    if (!amt || isNaN(Number(amt))) return;

    setIsProcessing(true);
    try {
      await submitRealStellarPayment(userData.publicKey, driver.publicKey, amt);
      alert(`Success! Manually pushed ${amt} XLM Fuel Credit allocation to ${driver.displayName}.`);
    } catch (err: any) {
      alert("Manual Push Failed: " + err.message);
    } finally {
              ...
              {approvedDrivers.map(d => (
                <tr key={d.id} style={{ borderBottom: `1px solid ${t.border}` }}>
                  ...
                  <td style={{ padding: "12px 0", textAlign: "right" }}>
                    <button onClick={() => handleSendFuelCreditDirect(d)} disabled={isProcessing} style={{ background: "#F59E0B", color: "#fff", border: "none", padding: "8px 16px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: isProcessing ? "not-allowed" : "pointer", opacity: isProcessing ? 0.6 : 1 }}>
                      Push Extra Credit
                    </button>
                  </td>
                </tr>
