...
const ADMIN_PUBLIC_KEY = "GADMINAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
const formatXlm = (value: number) => Number(value || 0).toFixed(2);
const dayMs = 24 * 60 * 60 * 1000;

type UserRole = "commuter" | "driver" | "cooperative";
...
const makePublicKey = (uid: string) => `G${uid.replace(/[^a-zA-Z0-9]/g, "").slice(0, 55)}`.padEnd(56, "A");

const toDurationDays = (value: number, unit: Policy["durationUnit"]) => {
  switch (unit) {
    case "weeks":
      return value * 7;
    case "months":
...
const scoreDeltaForVault = (lockedAmount: number, days: number) => {
  const base = Math.min(5, Math.ceil(lockedAmount / 20));
  return base + Math.min(5, Math.ceil(days / 14));
};

const maybeRunDailyTrustUpdate = async (userData: any) => {
  ...
  const loanSnap = await getDocsSafe(
    query(collection(db, "fuel_requests"), where("driverId", "==", userData.uid))
  );
  const overdue = loanSnap.some((item) => {
    const createdAt = parseTimestamp(item.createdAt);
    const durationDays = Number(item.durationDays || defaultPolicy.durationValue);
    return item.status === "active" && createdAt && now - createdAt.getTime() > durationDays * dayMs;
  });

  if (overdue) nextScore -= 4;

  const vaultSnap = await getDocsSafe(
    query(collection(db, "vaults"), where("ownerId", "==", userData.uid))
  );
  const matured = vaultSnap.some((item) => {
    const maturityDate = parseTimestamp(item.maturityDate);
    return item.status === "locked" && maturityDate && now >= maturityDate.getTime();
  });
...
const getDocsSafe = async (q: any) => {
  const snapshot = await new Promise<any>((resolve, reject) => {
    const unsub = onSnapshot(
      ...
      (result) => {
        unsub();
        resolve(result.docs.map((item: any) => ({ id: item.id, ...item.data() })));
      },
      reject
    );
  });
...
const queueBluetoothPayment = (userId: string, payload: any) => {
...
const syncBluetoothQueue = async (userId: string) => {
