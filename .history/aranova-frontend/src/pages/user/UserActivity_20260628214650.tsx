...
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const [isRequestingFuel, setIsRequestingFuel] = useState(false);
  ...
  useEffect(() => {
    // 1. Fetch Dynamic Fuel Credit Limit configured by the Driver's Cooperative from DB
    if (userData?.cooperativeId) {
      getDoc(doc(db, "cooperatives", userData.cooperativeId))
        .then((docSnap) => {
          if (docSnap.exists()) {
            setFuelLimit(docSnap.data().fuelCreditLimit || 150);
          } else {
            setFuelLimit(150); // Fallback limit
          }
        })
        .catch(() => setFuelLimit(150));
    }
  ...
  // 2. Soroban Smart Contract Invocation Simulation: issue_fuel_credit
  const handleRequestFuelCredit = async () => {
    if (isRequestingFuel) return;
    setIsRequestingFuel(true);

    try {
      console.log(`[Soroban Smart Contract Invoc] Calling issue_fuel_credit...`);
      console.log(`Params -> driver: ${userData.publicKey}, amount: ${fuelLimit}`);

      // Simulating passing raw contract requirements (require_auth, checking pool balance layer)
      await new Promise((resolve) => setTimeout(resolve, 2000));

      alert(`Smart Contract Execution Successful!\nMethod: issue_fuel_credit\nAllocated: ${fuelLimit} XLM has been transferred directly to the whitelisted Pump Asset receiver.`);
    } catch (err) {
      console.error(err);
      alert("Smart contract invocation failed or rejected.");
    } finally {
      setIsRequestingFuel(false);
    }
  };
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="dash-grid">
        ...
        <div className="dash-col-side" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderLeft: "4px solid #F59E0B", borderRadius: "0 20px 20px 0", padding: 20 }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 4, margin: "6px 0" }}>
              <span style={{ fontSize: 32, fontWeight: 900, letterSpacing: -1, color: "#F59E0B" }}>{fuelLimit}.00</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: t.textMuted, marginLeft: 2 }}>XLM Available</span>
            </div>
            <button
              onClick={handleRequestFuelCredit}
              disabled={isRequestingFuel}
              style={{ width: "100%", marginTop: 10, background: dark ? "#451A03" : "#FEF3C7", color: dark ? "#FCD34D" : "#92400E", border: "none", borderRadius: 8, padding: "8px", fontWeight: 700, fontSize: 12, cursor: "pointer", opacity: isRequestingFuel ? 0.6 : 1 }}
            >
              {isRequestingFuel ? "Executing Soroban Contract..." : "Request Fuel Advance"}
            </button>
          </div>
...
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  // State layers for updating DB configurations
  const [inputLimit, setInputLimit] = useState<number>(150);
  ...
  const fetchDriversAndConfig = async () => {
    try {
      setLoading(true);

      // Fetch dynamic fuel allowance configs set by this Cooperative entity
      const configSnap = await getDoc(doc(db, "cooperatives", userData.uid));
      if (configSnap.exists()) {
        setInputLimit(configSnap.data().fuelCreditLimit || 150);
      }
  ...
  useEffect(() => { fetchDriversAndConfig(); }, []);

  // 3. Send the updated dynamic ceiling limit directly to Firestore DB & call Soroban set_driver_limit logic
  const handleSaveFuelLimit = async () => {
    if (isSavingLimit) return;
    setIsSavingLimit(true);
    try {
      // Step A: Save metadata inside our Firestore configurations database layer
      await setDoc(doc(db, "cooperatives", userData.uid), {
        coopName: userData.coopName || userData.displayName,
        fuelCreditLimit: inputLimit,
        updatedAt: new Date().toISOString()
      }, { merge: true });

      // Step B: Soroban Smart contract state synchronization trigger simulation
      console.log(`[Soroban Smart Contract Sync] Invoking set_driver_limit...`);
      console.log(`Contract arguments -> admin: ${userData.uid}, limit: ${inputLimit}`);

      alert("Success! Daily driver credit ceiling updated in Firestore DB and synchronized with Aranova's on-chain Soroban parameters.");
    } catch (error) {
      console.error("Failed to save configuration layer:", error);
      alert("Error: Database configuration update rejected.");
    } finally {
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      {/* Set Fuel Credit Master Control Interface */}
      <div style={{ background: dark ? "#3B210B" : "#FFFBEB", border: `1px solid ${dark ? "#78350F" : "#FDE68A"}`, borderRadius: 16, padding: "16px 20px", marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <input
            ...
            style={{ width: 80, padding: "8px", borderRadius: 8, border: `1px solid ${dark ? "#78350F" : "#FDE68A"}`, background: dark ? "#451A03" : "#fff", color: t.textPrim, fontWeight: 700 }}
          />
          <button
            onClick={handleSaveFuelLimit}
            disabled={isSavingLimit}
            style={{ background: "#F59E0B", color: "#fff", border: "none", padding: "8px 16px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", opacity: isSavingLimit ? 0.6 : 1 }}
          >
      ...
      <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 20, padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <button onClick={fetchDriversAndConfig} style={{ background: "transparent", border: `1px solid ${t.border}`, color: t.textPrim, padding: "6px 12px", borderRadius: 8, fontSize: 12, cursor: "pointer", fontWeight: 700 }}>Refresh</button>
        </div>
