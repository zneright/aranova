...
// ---------------------------------------------------------------------------
// HELPER: SMART SIGNING ROUTER (AUTO-SIGN VS EXTENSION)
// ---------------------------------------------------------------------------
const signAndSubmitStellarTransaction = async (txBuilder: TransactionBuilder, userData: any, horizon: Horizon.Server) => {
  const tx = txBuilder.setTimeout(60).build();
  let signedTx;

  // Detect if wallet was generated internally (has secret key)
  const internalSecret = userData?.secretKey || localStorage.getItem(`aranova_secret_${userData?.uid}`);

  if (internalSecret) {
    // 1. AUTO-SIGN: No popups needed
    const keypair = Keypair.fromSecret(internalSecret);
    tx.sign(keypair);
    signedTx = tx;
  } else {
    // 2. EXTENSION-SIGN: Route to Freighter/xBull
            ...
            {pendingAllocationId ? (
              <div style={{ marginTop: 12 }}>
                ...
                <button onClick={handleClaimAllocationOnChain} disabled={isProcessing} style={{ width: "100%", background: "#10B981", color: "#fff", border: "none", borderRadius: 8, padding: "12px", fontWeight: 800, fontSize: 13, cursor: "pointer", opacity: isProcessing ? 0.6 : 1 }}>
                  {isProcessing ? "Signing Execution..." : "Instantly Liquidate Allocation"}
                </button>
              </div>
            ) : (
