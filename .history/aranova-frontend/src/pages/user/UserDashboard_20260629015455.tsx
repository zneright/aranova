...
import {
  ...
  nativeToScVal,
  xdr,
  rpc, // Replaced SorobanRpc with the correct rpc namespace
  Transaction
...
// Universal Wallet Kit Initialization
const kit = new StellarWalletsKit({
  network: WalletNetwork.TESTNET,
  selectedWalletId: "freighter", // Fallback, will intercept any active wallet
  modules: allowAllModules(),
});

// ---------------------------------------------------------------------------
// SVG Icons (Cleaned up unused icons)
// ---------------------------------------------------------------------------
...
const submitRealSorobanTransaction = async (publicKey: string, method: string, args: xdr.ScVal[]) => {
  ...
  tx = rpc.assembleTransaction(tx, simulated) as Transaction;

  // 3. Sign using StellarWalletsKit (Supports Freighter, xBull, Lobstr, etc.)
  const signedResult = await kit.signTransaction(tx.toXDR());
  const signedTx = TransactionBuilder.fromXDR(signedResult.signedXDR, NETWORK_PASSPHRASE);
