...
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const checkPendingOnChainAllocations = async () => {
    ...
    try {
      ...
      if (data && data._embedded && data.._embedded.records && data._embedded.records.length > 0) {
        const record = data._embedded.records[0]; // Fetch oldest un-claimed cooperative allocation
  ...
  const handleClaimAllocationOnChain = async () => {
    ...
    try {
      ...
      // Execute built-in programmatic asset claim protocol
      const tx = new TransactionBuilder(account, {
        fee: fee.toString(),
        networkPassphrase: NETWORK_PASSPHRASE
      })
...
// ---------------------------------------------------------------------------
// 3. COOPERATIVE DASHBOARD (PRE-AUTHORIZATION CEILING DEPOSITOR)
// ---------------------------------------------------------------------------
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const handleDepositPreAuthorizedAllocation = async (driver: any) => {
    if (!driver.publicKey) return alert("Driver public tracking key structure offline.");
    if (!userData.publicKey) return alert("Establish cooperative hardware wallet initialization parameters first.");

    const amt = window.prompt(`Enter amount of XLM allocation to load for ${driver.displayName}:`);
    ...
    try {
      ...
      try {
        await horizon.loadAccount(driver.publicKey);
      } catch (err: any) {
        if (err.name === "NotFoundError" || (err.response && err.response.status === 404)) {
      ...
      // Create a built-in programmatic escrow envelope restricted to this driver key
      const tx = new TransactionBuilder(account, {
        fee: fee.toString(),
        networkPassphrase: NETWORK_PASSPHRASE
      })
        .addOperation(Operation.createClaimableBalance({
          destinations: [
            { destination: driver.publicKey, predicate: ClaimPredicate.unconditional() }
          ],
          asset: Asset.native(),
          amount: amt.toString()
        }))
      ...
      alert(`Success! Pre-authorized budget threshold of ${amt} XLM deposited on-chain for ${driver.displayName}. They can now withdraw this asset autonomously.`);
    } catch (err: any) {
      alert("Escrow deposit failed: " + err.message);
    } finally { setIsProcessing(false); }
  };
    ...
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 20, padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h3 style={{ fontWeight: 800, fontSize: 16, color: t.textPrim }}>Fleet Fleet Status & Pre-Funding</h3>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${t.border}`, color: t.textMuted, fontSize: 13 }}>
                ...
                <th style={{ padding: "12px 0", textAlign: "right" }}>Voucher Setup</th>
              </tr>
            </thead>
            <tbody>
              {approvedDrivers.map(d => (
                <tr style={{ borderBottom: `1px solid ${t.border}` }} key={d.id}>
                  ...
                  <td style={{ padding: "12px 0", textAlign: "right" }}>
                    <button onClick={() => handleDepositPreAuthorizedAllocation(d)} disabled={isProcessing} style={{ background: "#F59E0B", color: "#fff", border: "none", padding: "8px 16px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: isProcessing ? "not-allowed" : "pointer", opacity: isProcessing ? 0.6 : 1 }}>
                      Load Pre-Approved Budget
                    </button>
                  </td>
                </tr>
              ))}
