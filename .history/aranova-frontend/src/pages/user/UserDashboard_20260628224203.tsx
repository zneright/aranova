...
// ---------------------------------------------------------------------------
// Global Constant Layer Config (UPDATED CONTRACT ID)
// ---------------------------------------------------------------------------
...
const SmartVaultWidget: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const handleUpdateVault = () => {
    console.log(`[Soroban Smart Contract] Executing configure_vault(user: ${userData.publicKey}, active: ${isActive}, bps: ${savePercentage * 100}, days: ${duration})`);
    alert(`Smart Vault synced on-chain! ${savePercentage}% of incoming payments will be securely locked to boost your Trust Score.`);
  };
...
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const executeFuelCreditContract = async () => {
    ...
    try {
      ...
      alert(`Fuel Credit Issued! ${requestedAmt} XLM automatically routed from Cooperative to Partner Pump.`);
      setCustomRequestAmount("");
    } catch { alert("Smart contract invocation failed."); }
  ...
  const executeRepayCreditContract = async () => {
    setIsProcessingContract(true);
    try {
      console.log(`[Soroban Smart Contract] Executing repay_fuel_credit(driver: ${userData.publicKey})`);
      console.log(`Math Matrix: Routing 0.2% fee to Admin, Principal + 0.3% yield to Cooperative.`);
      await new Promise(r => setTimeout(r, 1500));
      alert("Debt Settled On-Chain! Trust Score boosted by +5 points for timely repayment.");
      setTrustScore(prev => prev + 5);
    } catch { alert("Failed to settle debt. Ensure your wallet has sufficient XLM."); }
    finally { setIsProcessingContract(false); }
  };

  const triggerBluetoothPaymentReceive = () => {
    console.log(`[Soroban Emulation] process_payment(offline_mode) signature verified for ${ARANOVA_CONTRACT_ID}`);
    alert("Bluetooth Ad-Hoc Fare Received! Trust Score bumped +1.");
        ...
        <div className="dash-col-side" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderLeft: "4px solid #F59E0B", borderRadius: "0 20px 20px 0", padding: 20 }}>
            ...
            <button onClick={executeFuelCreditContract} disabled={isProcessingContract} style={{ width: "100%", background: dark ? "#451A03" : "#FEF3C7", color: dark ? "#FCD34D" : "#92400E", border: "none", borderRadius: 8, padding: "10px", fontWeight: 700, fontSize: 12, cursor: "pointer", opacity: isProcessingContract ? 0.6 : 1, marginBottom: 8 }}>
              {isProcessingContract ? "Processing..." : "Request Fuel Advance"}
            </button>
            ...
            <button onClick={executeRepayCreditContract} disabled={isProcessingContract} style={{ width: "100%", background: dark ? "#064E3B" : "#D1FAE5", color: dark ? "#34D399" : "#065F46", border: "none", borderRadius: 8, padding: "10px", fontWeight: 700, fontSize: 12, cursor: "pointer", opacity: isProcessingContract ? 0.6 : 1 }}>
              {isProcessingContract ? "Processing..." : "One-Click Settle Debt"}
            </button>
          </div>
        </div>
      </div>
...
const CommuterContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const triggerBluetoothOfflinePayment = () => {
    ...
    alert("Bluetooth Offline Ad-Hoc Mesh Payment successfully signed and delivered!");
    setTrustScore(prev => prev + 1);
  };
...
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const handleUpdateLimit = async () => {
    try {
      await setDoc(doc(db, "cooperatives", userData.uid), { fuelCreditLimit: coopLimitInput, updatedAt: new Date().toISOString() }, { merge: true });
      console.log(`[Soroban State Sync] set_driver_limit synchronized with contract: ${ARANOVA_CONTRACT_ID}`);
      alert("Cooperative ceiling metrics successfully set and verified on-chain.");
    } catch { alert("Failed to commit settings parameter."); }
  };

  const handleApprove = async (driver: any) => {
    try {
      console.log(`[Soroban] Executing set_driver_limit for approved driver: ${driver.publicKey || 'Unknown Key'}`);
      ...
      alert(`Driver ${driver.displayName} approved. Soroban limits configured!`);
    } catch (error) {
      console.error(error);
