...
// ---------------------------------------------------------------------------
// Global Constant Contract Layer Config
// ---------------------------------------------------------------------------
...
const DedicatedWalletManager: React.FC<{ userData: any; userRole: string }> = ({ userData, userRole }) => {
  ...
  return (
    <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 20, padding: 20, marginBottom: 20 }}>
      <button onClick={() => setShowManager(!showManager)} style={{ width: "100%", background: "none", border: "none", display: "flex", alignItems: "center", justify& Content: "space-between", cursor: "pointer", color: t.text, padding: 0 }}>
...
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const [coopCeiling, setCoopCeiling] = useState<number>(150);
  ...
  useEffect(() => {
    ...
    if (userData?.cooperativeId) {
      if (!isOnline) {
        ...
        if (offlineLimit) setCoopCeiling(Number(offlineLimit));
      } else {
        getDoc(doc(db, "cooperatives", userData.cooperativeId)).then(docSnap => {
          if (docSnap.exists()) {
            const limit = docSnap.data().fuelCreditLimit || 150;
          ...
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderLeft: "4px solid #F59E0B", borderRadius: "0 20px 20px 0", padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: t.textMuted, marginTop: 4 }}>Maximum limit parameter: {coopCeiling} XLM</div>
              </div>
            </div>
...
const CooperativeContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const [coopLimitInput, setCoopLimitInput] = useState<number>(150);
  ...
  const fetchDrivers = async () => {
    try {
      ...
      if (coopDoc.exists()) {
        setCoopLimitInput(coopDoc.data().fuelCreditLimit || 150);
      }
    ...
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      <div style={{ background: dark ? "#3B210B" : "#FFFBEB", border: `1px solid ${dark ? "#78350F" : "#FDE68A"}`, borderRadius: 16, padding: "16px 20px", marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: dark ? "#D97706" : "#B45309", marginTop: 4 }}>Set the max available XLM advance for verified drivers via Soroban parameters.</div>
        </div>
