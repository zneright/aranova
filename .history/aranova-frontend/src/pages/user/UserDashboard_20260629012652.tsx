...
const DriverContent: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const executeFuelCreditContract = async () => {
    ...
    try {
      // ── NEW: REAL TRANSACTION VERIFICATION (COOPERATIVE LIQUIDITY CHECK) ──
      const networkUrl = userData?.network === "TESTNET" ? "https://horizon-testnet.stellar.org" : "https://horizon.stellar.org";
      const coopRes = await fetch(`${networkUrl}/accounts/${coopWalletData}`);

      if (!coopRes.ok) {
        throw new Error("Network Error: Could not verify Cooperative wallet status on the blockchain.");
      }

      const coopData = await coopRes.json();
      const coopNativeBalance = coopData.balances.find((b: any) => b.asset_type === 'native');

      // Check if Cooperative actually has the money to fund this fuel transaction
      if (!coopNativeBalance || parseFloat(coopNativeBalance.balance) < requestedAmt) {
        throw new Error(`Transaction Blocked: The Cooperative's connected wallet lacks sufficient funds (Current Balance: ${coopNativeBalance ? parseFloat(coopNativeBalance.balance).toFixed(2) : "0"} XLM) to cover this ${requestedAmt} XLM fuel credit.`);
      }

      // ── REAL SOROBAN CALL: issue_fuel_credit ──
      const args = [
        new StellarSdk.Address(userData.publicKey).toScVal(),          // Driver initiating
        new StellarSdk.Address(coopWalletData).toScVal(),              // Cooperative paying
        new StellarSdk.Address(PARTNER_PUMP_PUBLIC_KEY).toScVal(),     // Gas Station receiving the 'Pay'
        new StellarSdk.Address(XLM_NATIVE_CONTRACT_ID).toScVal(),      // Asset
        StellarSdk.nativeToScVal(requestedAmt * 10000000, { type: 'i128' }) // Soroban uses stroops (7 decimals)
      ];

      await submitRealSorobanTransaction(userData.publicKey, "issue_fuel_credit", args);

      alert(`Transaction Verified! ${requestedAmt} XLM successfully drawn from the Cooperative wallet and paid directly to the Gas Station.`);
