...
const AuthPage: React.FC = () => {
  ...
  const connectSpecificWallet = async (walletId: string) => {
    ...
    try {
      ...
      if (!walletModule) {
        throw new Error(`The ${walletId} module could not be loaded. Check your imports.`);
      }

      // 2. Mobile/Web Safety Check: Ensure the wallet is actually available
      ...
      // 3. Call getAddress() directly on the ModuleInterface
      if (typeof walletModule.getAddress === 'function') {
        const response = await walletModule.getAddress();
        publicKey = response.address;
      }
      // Legacy fallback
      else if (typeof (walletModule as any).getPublicKey === 'function') {
        const response = await (walletModule as any).getPublicKey();
