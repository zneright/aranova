...
const AuthPage: React.FC = () => {
    ...
    try {
      if (authSuccess) {
        ...
        if (user) {
          ...
          // Merge to avoid overwriting existing data (like role)
        ...
        if (role === "commuter") approvedStatus = true;
        ...
        // User is authenticated, push them directly to Step 3
        setMode("login");
        setAuthSuccess(true);
      }
    } catch (err: any) {
  ...
  const switchMode = (m: AuthMode) => {
    ...
    setAuthSuccess(false); // Reset authentication flow state
  };
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", minHeight: "100vh", display: "grid", gridTemplateColumns: "1fr 1fr", background: "#F8F7F4" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", padding: "3rem", overflowY: "auto" }}>
        <div style={{ maxWidth: 460, width: "100%", margin: "0 auto" }}>
          ...
          {/* Hide the mode switcher if we are already in the post-auth Wallet setup phase */}
          {!authSuccess && (
          ...
          <p style={{ fontSize: 14, color: "#6B7280", margin: "0 0 1.75rem" }}>
            {authSuccess ? "Connect or generate a Stellar wallet to continue." : isLogin ? "Sign in to access your vault and offline payments." : "Step 1: Your Details"}
          </p>
          ...
          {/* ─── CREDENTIALS FIELDS (Hidden once authenticated) ─── */}
          {!authSuccess && (
            <>
              {!isLogin && (
          ...
          {/* ─── ONBOARDING STEPS FOR SIGNUP (Hidden once authenticated) ─── */}
          {!isLogin && !authSuccess && (
            <>
          ...
          {/* ─── POST-AUTH: WALLET INTEGRATION (Shows ONLY after successful authentication) ─── */}
          {authSuccess && (
            <>
              <div style={{ display: "flex", gap: "10px", marginBottom: "1rem" }}>
