...
const AuthPage: React.FC = () => {
  ...
  // --- BULLETPROOF Universal Wallet Connection Logic ---
  const handleConnectExtension = async () => {
    setWalletMode("connect");
    setErrors({ ...errors, wallet: "" });

    try {
      // Access the wildcard import safely to prevent Vite static analysis crashes
      const sdk = SWK as any;

      // Load modules dynamically depending on what the installed version supports
      const modules = sdk.allowAllModules
        ? sdk.allowAllModules()
        : sdk.defaultModules
          ? sdk.defaultModules()
          : [
            sdk.FreighterModule && new sdk.FreighterModule(),
            sdk.xBullModule && new sdk.xBullModule(),
            sdk.LobstrModule && new sdk.LobstrModule(),
          ].filter(Boolean);

      // Support both new static .init() and old Class constructors
      const kit = sdk.StellarWalletsKit.init
        ? sdk.StellarWalletsKit.init({ network: "PUBLIC", modules })
        : new sdk.StellarWalletsKit({ network: "PUBLIC", selectedModuleId: 'freighter', modules });
