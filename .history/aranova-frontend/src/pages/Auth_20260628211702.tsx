...
type ForgotMode = "none" | "email" | "code" | "reset";
...
const AuthPage: React.FC = () => {
  ...
  // Forgot Password State (Mocks for the 6-digit flow)
  ...
  const connectSpecificWallet = async (walletId: string) => {
    ...
    try {
      ...
      // Initialize modules without forcing a network
      ...
      // Auto-detect the active network of the wallet. Fallback to PUBLIC if it can't be read.
      let activeNetwork = "PUBLIC";
      if (typeof walletModule.getNetwork === "function") {
        const detected = await walletModule.getNetwork();
        if (detected) activeNetwork = detected.toUpperCase();
      }
      ...
      // ─── SIGNATURE REQUEST (Network Agnostic / Auto-Detected) ───
      try {
        const authMessage = `Aranova Authentication\n\nPlease sign to verify ownership.\nTimestamp: ${Date.now()}`;

        const currentNetworkPassphrase = activeNetwork === "PUBLIC"
    ...
    try {
      ...
      if (user) {
        await setDoc(doc(db, "users", user.uid), {
          walletCreated: true,
          publicKey: pubKey,
          network: networkString // Save the auto-detected network
        }, { merge: true });
  ...
  const handleSendResetCode = () => {
    if (!form.email) {
      setErrors({ email: "Please enter your email to receive the code." });
      return;
    }
    setLoading(true);
    // MOCK: Generate 6-digit code. In production, call your Cloud Function here.
    ...
    setTimeout(() => {
      ...
      setForgotMode("code");
      setErrors({});
      // Automatically log the code for testing purposes
      console.log(`[TESTING] Your reset code is: ${code}`);
    }, 1000);
  };

  const handleVerifyResetCode = () => {
    if (resetCodeInput !== mockGeneratedCode) {
  ...
  const handleResetPasswordSubmit = () => {
    ...
    setLoading(true);
    // MOCK: In production, you would use a Cloud Function to reset the password based on the verified code/email, 
    // OR if the user is somehow authenticated, use updatePassword(auth.currentUser, form.password).
    // Because Firebase client SDK doesn't support unauthenticated custom password changes, you need a backend endpoint here.
    setTimeout(() => {
      setLoading(false);
      setForgotMode("none");
      setMode("login");
      setErrors({ general: "Password successfully reset. You can now log in." }); // Reusing general error block as a success message for simplicity
    }, 1000);
  };
      ...
      if (authSuccess) {
        ...
        if (user) {
          ...
          if (walletMode === "create" && generatedKeys) {
            ...
            // Local wallets can default to PUBLIC since they are auto-generated offline keys
  ...
  const switchMode = (m: AuthMode) => {
    setMode(m); setErrors({}); setAuthSuccess(false); setForgotMode("none");
            ...
            {forgotMode !== "none" ? (
              <>
                <h1 style={{ fontSize: 26, fontWeight: 900, color: "#111827", margin: "0 0 8px", letterSpacing: "-0.5px" }}>
                  {forgotMode === "email" ? "Reset Password" : forgotMode === "code" ? "Enter Security Code" : "Set New Password"}
                </h1>
                <p style={{ fontSize: 15, color: "#6B7280", margin: "0 0 2rem", lineHeight: 1.5 }}>
                  {forgotMode === "email" ? "Enter your email to receive a 6-digit verification code." : forgotMode === "code" ? "We've sent a 6-digit code to your email." : "Secure your account with a new password."}
                </p>
                ...
                {forgotMode === "email" && (
                  <div style={{ marginBottom: "1.25rem" }}>
                    ...
                    <button onClick={handleSendResetCode} disabled={loading} style={{ ...S.primaryBtn, marginTop: "2rem", opacity: loading ? 0.7 : 1 }}>
                ...
                {forgotMode === "code" && (
                  <div style={{ marginBottom: "1.25rem" }}>
                    ...
                    <input style={{ ...S.input, borderColor: errors.code ? "#EF4444" : "#E5E7EB", letterSpacing: "4px", fontSize: 20, textAlign: "center" }} type="text" placeholder="000000" maxLength={6} value={resetCodeInput} onChange={(e) => setResetCodeInput(e.target.value.replace(/[^0-9]/g, ''))} />
                    {errors.code && <p style={{ color: "#EF4444", fontSize: 12, margin: "6px 0 0" }}>{errors.code}</p>}
                    <button onClick={handleVerifyResetCode} style={{ ...S.primaryBtn, marginTop: "2rem" }}>
                      Verify Code
                    </button>
                  </div>
                )}
                ...
                {forgotMode === "reset" && (
                  <>
                    ...
                    <button onClick={handleResetPasswordSubmit} disabled={loading} style={{ ...S.primaryBtn, opacity: loading ? 0.7 : 1 }}>
              ...
              <>
                ...
                {/* Simulated Success Message injection */}
                {errors.general && (
                  <div style={{ background: errors.general.includes("success") ? "#D1FAE5" : "#FEE2E2", color: errors.general.includes("success") ? "#065F46" : "#991B1B", padding: "12px 16px", borderRadius: "12px", marginBottom: "1.5rem", fontSize: "14px", fontWeight: 600, textAlign: "center" }}>
                    {errors.general}
                  </div>
                )}
