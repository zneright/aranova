...
const AuthPage: React.FC = () => {
  ...
  const connectSpecificWallet = async (walletId: string) => {
    ...
    try {
      ...
      // Initialize wallet modules bypassing TS conflicts across versions
      if (walletId === 'freighter') {
        // @ts-ignore
        walletModule = new FreighterModule({ network });
      } else if (walletId === 'xbull') {
        // @ts-ignore
        walletModule = new xBullModule({ network });
      } else if (walletId === 'lobstr') {
        // @ts-ignore
        walletModule = new LobstrModule({ network });
      }
      ...
      try {
        ...
        // Some kit versions wrap params in an object, older ones use positional args. 
        // We use the common pattern for stellar-wallets-kit v1+ 
        ...
        // Correctly handle JSON/Object errors to avoid React Error #31
    ...
    try {
      ...
      // ─── STAGE 1: SIGNING UP ───
        ...
        const userData: any = {
          ...
          network: network // Save selected network!
        };
