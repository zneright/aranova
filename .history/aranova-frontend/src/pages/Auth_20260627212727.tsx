...
const AuthPage: React.FC = () => {
  ...
  const handleConnectExtension = async () => {
    ...
    setErrors({ ...errors, wallet: "" }); // Clear previous errors

    try {
      // 1. Check if the user actually has the extension installed
      const hasExtension = await isConnected();

      if (hasExtension) {
        // 2. This triggers the actual browser pop-up!
        const publicKey = await getPublicKey();

        // 3. If they approve, save the key to the state
        setConnectPubKey(publicKey);
      } else {
        setErrors({
          ...errors,
          wallet: "Freighter extension not found. Please install the Freighter browser extension."
        });
      }
    } catch (error) {
      // If the user clicks "Reject" in the pop-up, it throws an error
      console.error("User rejected the connection or it failed:", error);
      setErrors({
        ...errors,
        wallet: "Connection request was rejected. Please try again."
      });
    }
  };
