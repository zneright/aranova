...
const AuthPage: React.FC = () => {
  ...
  // FETCH REAL COOPERATIVES FROM FIRESTORE
  useEffect(() => {
    const fetchCoops = async () => {
      try {
        const q = query(
          ...
          where("approved", "==", true) // ONLY fetch coops that are verified by admin
        );
  ...
  const handleGenerateWallet = () => {
    ...
    // Pick 3 random unique words to verify
    ...
    try {
      ...
      // ─── STAGE 1: EXISTING USER LOGIN ───
        ...
        if (userDoc.exists()) {
          ...
          // Logic: Only show Wallet Creation if they are Approved AND haven't made a wallet yet
          if (userData.approved === true && userData.walletCreated === false) {
            setAuthSuccess(true);
          } else {
            // If they are pending (approved = false) OR already have a wallet, go to dashboard
            navigate(userData.role === "cooperative" ? "/admin" : "/user");
          }
        }
      }
      // ─── STAGE 1 & 2: NEW USER SIGNUP ───
        ...
        const isApproved = role === "commuter"; // Commuters bypass pending phase
        ...
        if (isApproved) {
          // Commuters proceed immediately to Wallet Setup
          setMode("login");
          setAuthSuccess(true);
        } else {
          // Drivers and Cooperatives bypass Wallet Setup for now, sent to Dashboard waiting room
          navigate(role === "cooperative" ? "/admin" : "/user");
        }
      }
    } catch (err: any) {
