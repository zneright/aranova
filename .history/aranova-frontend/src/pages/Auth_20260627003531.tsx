...
const AuthPage: React.FC = () => {
  ...
  const [verificationStep, setVerificationStep] = useState(0); // 0: initial, 1, 2, 3: word checks
  ...
  const validate = () => {
    ...
    if (!isLogin) {
      ...
      // Enforce at least 10 characters for phone numbers
  ...
  const startVerification = () => {
    const indices = [Math.floor(Math.random() * 4), Math.floor(Math.random() * 4) + 4, Math.floor(Math.random() * 4) + 8];
