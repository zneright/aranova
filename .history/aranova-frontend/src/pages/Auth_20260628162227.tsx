...
const AuthPage: React.FC = () => {
  ...
  // --- FINALLY BULLETPROOF Universal Wallet Connection Logic ---
  const handleConnectExtension = async () => {
    ...
    try {
      ...
      // Definitively resolve the modules safely
      let activeModules = [];
      if (typeof sdk.allowAllModules === "function") {
        activeModules = sdk.allowAllModules();
      } else {
        if (typeof sdk.FreighterModule === "function") activeModules.push(new sdk.FreighterModule());
        if (typeof sdk.xBullModule === "function") activeModules.push(new sdk.xBullModule());
        if (typeof sdk.LobstrModule === "function") activeModules.push(new sdk.LobstrModule());
      }

      // Definitively grab the correct Class reference
      const KitClass = sdk.StellarWalletsKit || (sdk.default && sdk.default.StellarWalletsKit);

      if (!KitClass) {
        throw new Error("StellarWalletsKit could not be found in the package.");
      }

      // Explicitly instantiate it using 'new'
      ...
      // Confirm openModal exists before calling it
      if (!kit || typeof kit.openModal !== "function") {
        throw new Error("Initialization failed or modal function is missing.");
      }

      // Open the beautiful modal
      await kit.openModal({
        onWalletSelected: async (option: any) => {
          try {
            ...
            // Immediately auto-submit and forward the user inside
            await autoSubmitExternalWallet(publicKey);

          } catch (err) {
            console.error("Wallet connection error:", err);
            setErrors({ ...errors, wallet: "Connection failed. Please ensure the extension is unlocked." });
          }
        }
      });
    } catch (error: any) {
      console.error("Modal closed or failed:", error);
      setErrors({ ...errors, wallet: "Wallet connection was cancelled or failed." });
    }
  };
      ...
      <div className="auth-layout">
        ...
        <div className="auth-right">
          <div className="auth-right-content">
              ...
              <>
                {(!walletMode || walletMode === "connect") && (
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "2rem" }}>
                    ...
                    {errors.wallet && <p style={{ color: "#EF4444", fontSize: 13, fontWeight: 700, margin: "4px 0 0", textAlign: "center" }}>{errors.wallet}</p>}
                  </div>
                )}
            ...
            {(!authSuccess || (walletMode === "create" && verificationStep === 4)) && (
