...
const AuthPage: React.FC = () => {
  ...
  const connectSpecificWallet = async (walletId: string) => {
    ...
    try {
      ...
      // 1. Instantiate ONLY the specific module directly (Bypasses kit wrapper conflicts)
      if (walletId === 'freighter' && typeof sdk.FreighterModule === "function") {
        walletModule = new sdk.FreighterModule();
      } else if (walletId === 'xbull' && typeof sdk.xBullModule === "function") {
        walletModule = new sdk.xBullModule();
      } else if (walletId === 'lobstr' && typeof sdk.LobstrModule === "function") {
      ...
      // 2. Mobile/Web Safety Check: Ensure the wallet is actually available in this environment
      const isAvailable = await walletModule.isAvailable();
      if (!isAvailable) {
        throw new Error(
          `${walletId === 'freighter' ? 'Freighter' : walletId === 'xbull' ? 'xBull' : 'LOBSTR'} ` +
          `is not installed or not supported on this specific browser/device.`
        );
      }

      let publicKey = "";

      // 3. Call getAddress() directly on the ModuleInterface (Guaranteed to exist by documentation specs)
      if (typeof walletModule.getAddress === 'function') {
        const response = await walletModule.getAddress();
        publicKey = response.address;
      }
      // Legacy fallback just in case an older version was cached
        ...
        throw new Error("The wallet module did not provide a getAddress method.");
      }
      ...
      // Format a clean error for the UI so mobile users understand what happened
      setErrors({
        ...errors,
