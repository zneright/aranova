...
type WalletMode = "create" | "connect" | null; // Removed 'restore'
...
const AuthPage: React.FC = () => {
  ...
  const handleConnectExtension = async () => {
    ...
    try {
      console.log("1. Checking if Freighter is installed...");
      const installed = await isConnected();

      if (!installed) {
        console.error("Freighter is not detected by the browser.");
        setErrors({
          ...errors,
          wallet: "Freighter extension not found. Please install and unlock it."
        });
        return;
      }

      console.log("2. Freighter detected. Requesting access (this should trigger the pop-up)...");
      // Explicitly request access before asking for the key
      const accessResponse = await requestAccess();

      if (accessResponse.error) {
        console.error("Access request failed:", accessResponse.error);
        setErrors({ ...errors, wallet: accessResponse.error });
        return;
      }

      console.log("3. Access granted! Fetching public key...");
      const publicKey = await getPublicKey();

      if (publicKey) {
        console.log("4. Success! Public Key retrieved:", publicKey);
        setConnectPubKey(publicKey);
      } else {
        setErrors({ ...errors, wallet: "Failed to retrieve public key." });
      }
      ...
      setErrors({
        ...errors,
        wallet: "Connection request was rejected or failed. Check the browser console."
      });
    }
  };
  ...
  const validate = () => {
    ...
    if (authSuccess) {
      ...
      if (walletMode === "connect" && connectPubKey.length !== 56) {
        e.wallet = "Invalid Stellar Public Key length. Ensure the extension successfully connected.";
      }
  ...
  // Persistent Context-Aware Back Handler
  const handleBack = () => {
    if (authSuccess) {
      if (walletMode) {
        // Back out of specific wallet creation flow to wallet options
        setWalletMode(null);
        setVerificationStep(0);
        setGeneratedKeys(null);
        setConnectPubKey("");
      } else {
        // Log out the user if they abort wallet creation entirely
        signOut(auth).then(() => {
      ...
      // From login, theoretically routes back to a landing page (or is hidden)
      navigate("/");
    }
  };
  ...
  return (
    <>
      <style>{`
        ...
        .auth-left { background: linear-gradient(145deg, #0D2A6E 0%, #1652C9 50%, #0f1f54 100%); display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 3rem 1.5rem; position: relative; overflow: hidden; }
        .auth-right { display: flex; flex-direction: column; padding: 2rem 1.25rem; flex: 1; justify-content: center; background: #ffffff; }
        .auth-right-content { max-width: 480px; width: 100%; margin: 0 auto; padding-bottom: 2rem; position: relative; }
        .vehicle-icons { display: none; }
        
        /* Mobile specific fixes */
        @media (max-width: 767px) {
           .auth-left { padding: 2rem 1.5rem; min-height: 250px; flex: none; }
           .auth-right { padding: 2rem 1rem; border-radius: 24px 24px 0 0; margin-top: -20px; z-index: 10; box-shadow: 0 -4px 20px rgba(0,0,0,0.05); }
        }

        @media (min-width: 768px) {
          .auth-layout { display: grid; grid-template-columns: 1fr 1.2fr; }
          .auth-left { padding: 4rem 2rem; justify-content: center; }
          .auth-right { padding: 4rem 3rem; overflow-y: auto; background: #F8F7F4; align-items: center; }
        ...
        <div className="auth-right">
          <div className="auth-right-content">
            ...
            <p style={{ fontSize: 15, color: "#6B7280", margin: "0 0 2rem", lineHeight: 1.5 }}>
              {authSuccess ? "Connect an extension or generate a wallet to continue." : isLogin ? "Sign in to access your vault and offline payments." : "Step 1: Core Details"}
            </p>
              ...
              <>
                {(!walletMode || walletMode === "connect") && (
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "2rem" }}>
                    ...
                    <button onClick={handleConnectExtension} style={{ ...S.ghostBtn, background: walletMode === "connect" ? "#1652C9" : "#fff", color: walletMode === "connect" ? "#fff" : "#111827", borderColor: walletMode === "connect" ? "#1652C9" : "#E5E7EB" }}>
                      Connect Extension (Lobstr/Freighter)
                    </button>
                  </div>
                )}
                ...
                {walletMode === "create" && generatedKeys && (
                  <div style={{ background: "#F9FAFB", border: "1px solid #E5E7EB", borderRadius: 16, padding: "24px", marginBottom: "2rem" }}>
                    ...
                    {errors.wallet && <p style={{ color: "#EF4444", fontSize: 13, fontWeight: 700, margin: "12px 0 0" }}>{errors.wallet}</p>}
                  </div>
                )}

                {walletMode === "connect" && (
                  <div style={{ background: "#F9FAFB", border: "1px solid #E5E7EB", borderRadius: 16, padding: "24px", marginBottom: "2rem" }}>
                    <h4 style={{ margin: "0 0 12px", fontSize: 16, fontWeight: 900 }}>1. Connect Browser Wallet</h4>
                    <p style={{ fontSize: 13, color: "#4B5563", marginBottom: 16, lineHeight: 1.5 }}>
                      Please approve the connection in your Freighter or Lobstr extension. If the extension doesn't pop up, you can manually input your Stellar Public Key below.
                    </p>
            ...
            {(!authSuccess || verificationStep === 4 || walletMode === "connect") && (
